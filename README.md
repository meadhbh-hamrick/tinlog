...
sn-logger is an attempt to get something like the old VMS message utility
...
<pre>    npm install sn-logger</pre>

If you want to check out the source, use the git command:

<pre>    git clone git://github.com/smithee-us/sn-logger.git</pre>
...
<pre>var logger = require( 'sn-logger' );
var descriptor = {
    facility: 'EXAMPLE',
    messages: {
      I: {
...
When sn-logger sees a messages_path element, it attempts to open that
...
For historical reasons, messages created by sn-logger do not include a
